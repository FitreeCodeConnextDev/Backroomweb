...
class UsersController extends Controller
{
    ...
    public function store(UserInfoRequest $request)
    {
        try {
            ...
            $user_info->user_pass = $request->user_pass;
            $user_info->branch_id = $request->branch_id;
            $user_info->city_license = $request->city_license;
            $user_info->card_no = $request->card_no;
            $user_info->needresetpass = $request->needresetpass;
            $user_info->resetpass_day = $request->resetpass_day;
            $user_info->user_nomiss = 0;
            $user_info->issuedate = Carbon::now()->toDateString();
            $user_info->issuetime = Carbon::now()->toDateTimeString();
            $user_info->front_permiss = $front_merge;
            $user_info->back_permiss = $back_merge;
            $user_info->term_permiss = $vendor_merge;
            $user_info->function_permiss = $permiss_merge;
            $user_info->lastuser_pass = $request->user_pass;
            $user_info->startdate_reset = Carbon::now()->toDateTimeString();
            $user_info->activeflag = 1;
            $user_info->user_lock = 0;

            if ($user_info->save()) {
                Log::channel('activity')->notice(session('auth_user.user_id') . ' Created a new user: ' . $request->user_id, [
            ...
            Log::channel('activity')->error(session('auth_user.user_id') . 'Failed to create a new user: ' . $request->user_id, [
                'user_id' => $request->user_id,
                'user_name' => $request->user_name,
                'branch_id' => $request->branch_id,
                'action' => 'create',
    ...
    public function update(UserInfoRequest $request, $id)
    {
        try {
            ...
            $user_info = UserInfoModel::find($id);
            $user_info->user_name = $request->user_name;
            $user_info->user_pass = $request->user_pass;
            $user_info->branch_id = $request->branch_id;
            $user_info->city_license = $request->city_license;
    ...
    public function changePassword(Request $request)
    {
        ...
        try {
            DB::table('user_info')->where('user_id', $userId)
                ->update([
                    'user_pass' => $request->input('new_password'),
                    'lastuser_pass' => $request->input('current_password'),
                    'startdate_reset' => Carbon::now()->toDateTimeString(),
                ]);
