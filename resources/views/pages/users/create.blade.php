...
@section('form-section')
    <form action="{{ route('users.store') }}" method="post" id="users_form" enctype="multipart/form-data">
        @csrf
        <div class="grid lg:grid-cols-2 grid-cols-1 gap-6 mb-4">
            <div>
                ...
                <input type="text" id="user_id" name="user_id" class="input_text"
                    placeholder="{{ __('users.user_id') }}" value="{{ old('user_id') }}" required />
                @error('user_id')
            ...
            @else
                <div>
                    ...
                    <input type="text" id="branch_id" name="branch_id" class="input_text"
                        placeholder="{{ __('users.branch_id') }}" readonly value="{{ session('auth_user.branch_id') }}"
                        required />
                    @error('branch_id')
                        <p class="mt-2 text-sm text-red-600 "><span class="font-medium"> {{ __('menu.is_warning') }} </span>
                            {{ $message }}
            ...
            <div>
                ...
                <input type="text" id="user_name" name="user_name" class="input_text"
                    placeholder="{{ __('users.user_name') }}" value="{{ old('user_name') }}" required />
                @error('user_name')
                    <p class="mt-2 text-sm text-red-600 "><span class="font-medium"> {{ __('menu.is_warning') }} </span>
                        {{ $message }}
                    </p>
            ...
            <div>
                ...
                <input type="text" id="city_license" name="city_license" class="input_text"
                    placeholder="{{ __('users.city_license') }}" value="{{ old('city_license') }}" />
            </div>
            <div>
                <label for="user_pass" class="label_input">{{ __('users.user_pass') }}</label>
                <input type="password" id="user_pass" name="user_pass" class="input_text"
                    placeholder="{{ __('users.user_pass') }}" value="{{ old('user_pass') }}" required />
                @error('user_pass')
                    <p class="mt-2 text-sm text-red-600 "><span class="font-medium"> {{ __('menu.is_warning') }} </span>
                        {{ $message }}
                    </p>
            ...
            <div>
                ...
                <input type="text" id="card_no" name="card_no" class="input_text"
                    placeholder="{{ __('users.card_no') }}" value="{{ old('card_no') }}" />
            </div>

            <div class="grid lg:grid-cols-2 grid-cols-1 gap-2">
                <div class="mt-5">
                    <input id="default-checkbox" type="hidden" name="needresetpass" value="0">
                    <input id="needresetpass" type="checkbox" name="needresetpass" value="1" class="checkbox_input">
                    <label for="needresetpass" class="label_checkbox">{{ __('users.needresetpass') }}</label>
                </div>
                <section class="mt-2">
                    <input type="number" id="resetpass_day" name="resetpass_day" class="input_text" value="0"
                        placeholder="{{ __('users.resetpass_day') }}  " />
                </section>
            </div>
    ...
    <script>
        $('#users_form').validate({
            ...
            messages: {
                ...
                user_pass: {
                    required: `{{ __('users.user_pass_required') }}`
                },
            }
        })
    </script>
