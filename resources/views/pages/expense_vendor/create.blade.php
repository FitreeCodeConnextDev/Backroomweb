...
@endsection
@php
    $maxExpCode = DB::table('expensevendor_info')->max('exp_code');
    $nextExpCode = $maxExpCode ? (int) $maxExpCode + 1 : 1;
    $exp_code = str_pad($nextExpCode, 2, '0', STR_PAD_LEFT);
@endphp
@section('form-section')
    <form action="{{ route('expense_vendor.store') }}" id="expense_vendor" method="post">
        @csrf
        <div class="grid_page">
            <div>
            ...
            <div>
                ...
                <input type="number" id="price_rate" name="price_rate" placeholder="..." class="input_text"
                    value="" />
                @error('price_rate')
                    <p class="mt-2 text-sm text-red-600 "><span class="font-medium"> {{ __('menu.is_warning') }} </span>
                        {{ $message }}
