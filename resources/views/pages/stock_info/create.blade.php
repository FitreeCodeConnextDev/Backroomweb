    ...
    <form id="stock_info" action="{{ route('stock-info.store') }}" method="post">
        ...
        <div class="grid_page">
            ...
            <div>
                ...
                <input type="date" id="txndate" name="txndate" class="input_text" value="" required />
                @error('txndate')
                    <p class="mt-2 text-sm text-red-600"><span class="font-medium"> {{ __('menu.is_warning') }}
                        </span>{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="text" id="remark" name="remark" class="input_text" value="" required />
                @error('remark')
                    <p class="mt-2 text-sm text-red-600"><span class="font-medium"> {{ __('menu.is_warning') }}
                        </span>{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="date" class="input_text" id="refdate" name="refdate" value=" " required>
                @error('refdate')
                    <p class="mt-2 text-sm text-red-600"><span class="font-medium"> {{ __('menu.is_warning') }}
                        </span>{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <select name="txn_type" id="txn_type" class="input_text" required>
                    <option value="00" selected>{{ __('stock.txn_type_00') }}</option>
                    <option value="01">{{ __('stock.txn_type_01') }}</option>
                </select>
                @error('txn_type')
                    <p class="mt-2 text-sm text-red-600"><span class="font-medium"> {{ __('menu.is_warning') }}
                        </span>{{ $message }}</p>
        ...
        <a type="button" id="cancel_product" href="{{ route('stock-info.index') }}">
    ...
    <script type="module">
        ...
        $('#stock_info').validate({
            ...
            submitHandler: function(form) {
                const txnno = document.getElementById('txnno').value;
                const dataProduct = JSON.parse(localStorage.getItem('data_product')) || [];
                const filteredProducts = dataProduct.filter(item => item.txnno === txnno);

                if (filteredProducts.length === 0) {
                    Swal.fire({
                        icon: 'warning',
                        title: '{{ __('menu.is_warning') }}',
                        text: '{{ __('stock.no_data_product') }}', // Ensure this translation key exists or use a generic one
                ...
                document.getElementById('products_json').value = JSON.stringify(filteredProducts);
                form.submit();
            }
        });
    </script>
