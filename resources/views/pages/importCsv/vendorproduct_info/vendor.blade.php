...
@section('title_page')
    {{ __('import.import_vendorproduct_desc') }}
@endsection
@section('index-title')
    {{ __('import.import_vendorproduct_desc') }}
@endsection
@section('table-section')
    <form action="{{ route('import.vendorproduct.preview') }}" enctype="multipart/form-data" method="post">
        @csrf
        <div class="flex gap-4 p-4">
            <!-- Left side select -->
            <div class="w-1/2">
                <select name="from[]" size="8" id="mySideToSideSelect" class="w-full p-2 border rounded" multiple>
                    @foreach ($vendor_col as $column_name)
...
@section('js-scripts')
    ...
    <script>
        function saveSelectedToLocalStorage() {
            ...
            localStorage.setItem('selectedVendorProductColumns', JSON.stringify(selectedValues));
        }
        ...
        ['mySideToSideSelect_rightAll', 'mySideToSideSelect_rightSelected', 'mySideToSideSelect_leftSelected',
            'mySideToSideSelect_leftAll'
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const saved = localStorage.getItem('selectedVendorProductColumns');
            if (saved) {
