...
@section('title_page')
    {{ __('import.import_user_desc') }}
@endsection
@section('index-title')
    {{ __('import.import_user_desc') }}
@endsection
@section('table-section')
    <form action="{{ route('import.user.preview') }}" enctype="multipart/form-data" method="post">
        @csrf
        <div class="flex gap-4 p-4">
            <!-- Left side select -->
            <div class="w-1/2">
                <select name="from[]" size="8" id="mySideToSideSelect" class="w-full p-2 border rounded" multiple>
                    @foreach ($user_col as $column_name)
...
@section('js-scripts')
    ...
    <script>
        function saveSelectedToLocalStorage() {
            ...
            localStorage.setItem('selectedUserColumns', JSON.stringify(selectedValues));
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const saved = localStorage.getItem('selectedUserColumns');
            if (saved) {
