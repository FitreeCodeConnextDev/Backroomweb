@php
    $search = request()->input('vendorproduct_promo_search', '');
    $vendorproduct_promo = DB::table('vendorproductpromotion_info')
        ...
        ->when($search, function ($query) use ($search) {
            $query->where(function ($subQuery) use ($search) {
                $subQuery
                    ->where('vendorproductpromotion_info.start_date', 'like', '%' . $search . '%')
                    ->orWhere('product_info.product_desc', 'like', '%' . $search . '%')
                    ->orWhere('vendorproductpromotion_info.end_date', 'like', '%' . $search . '%');
            });
        })
        ->paginate(10)
        ->appends(['vendorproduct_promo_search' => $search]);
    ...
    <div class="grid grid-cols-1 gap-3">
        ...
        <div class="overflow-x-auto">
            <div class="flex justify-start mb-3">
                <form method="GET">
                    <input placeholder="Search..." name="vendorproduct_promo_search" value="{{ $search }}"
                ...
                <div class="grid gap-4 mb-4 grid-cols-1 lg:grid-cols-2">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="time" pattern="[0-9]{2}:[0-9]{2}" id="start_time" name="start_time"
                                class="input_text" />
                        </div>
                    </div>
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="time" pattern="[0-9]{2}:[0-9]{2}" id="end_time" name="end_time"
                                class="input_text" />
                        </div>
                    </div>
                </div>
