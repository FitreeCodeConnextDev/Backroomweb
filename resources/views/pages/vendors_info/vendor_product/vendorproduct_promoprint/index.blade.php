    ...
    $vendorpromo = DB::table('vendorpromotionprint_info')
        ...
        ->when($search, function ($query) use ($search) {
            $query->where(function ($subQuery) use ($search) {
                $subQuery
                    ->where('vendorpromotionprint_info.promo_seq', 'like', '%' . $search . '%')
                    ->orWhere('product_info.product_desc', 'like', '%' . $search . '%')
                    ->orWhere('vendorpromotionprint_info.description1', 'like', '%' . $search . '%')
                    ->orWhere('vendorpromotionprint_info.description2', 'like', '%' . $search . '%');
            });
        })
        ->paginate(10)
