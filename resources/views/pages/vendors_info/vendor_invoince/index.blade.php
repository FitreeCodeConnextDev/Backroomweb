                ...
                <div class="mt-3 border border-gray-200 rounded-lg p-5">
                    <div class="grid grid-cols-1 gap-3 py-3">
                        ...
                        <section>
                            ...
                            <textarea name="invoiceaddr1" class="input_text" id="" required
                                @if (Route::is('vendor-page.show')) disabled @endif>{{ $vendor_invoice->invoiceaddr1 }}</textarea>
                        </section>
                        <section>
                            <label for="invoiceaddr2" class="label_input"> {{ __('vendor.invoice_addr') }} </label>
                            <textarea name="invoiceaddr2" class="input_text" @if (Route::is('vendor-page.show')) disabled @endif>{{ $vendor_invoice->invoiceaddr2 }}</textarea>
                        </section>
                        <section>
                            ...
                            <select name="invoiceduedate" id="invoiceduedate" class="input_text" required
                                ...
                                @for ($i = 1; $i <= $daysInMonth; $i++)
                                    <option value="{{ $i }}"
                                        @if ($vendor_invoice->invoiceduedate == $i) selected @endif>
                                        {{ $i }}
                                    </option>
                                @endfor
                        ...
                        <section>
                            ...
                            <select name="invoicepaydate" id="invoicepaydate" class="input_text" required
                                ...
                                @for ($i = 1; $i <= $daysInMonth; $i++)
                                    <option value="{{ $i }}"
                                        @if ($vendor_invoice->invoicepaydate == $i) selected @endif>
                                        {{ $i }}
                                    </option>
                                @endfor
                            </select>
