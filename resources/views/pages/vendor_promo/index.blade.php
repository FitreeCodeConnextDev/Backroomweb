...
@section('add-button')
    <div>
        <a href="{{ route('vendor-promotion.create') }}" class="add-button">
...
@section('table-section')
    <table class="table-data" id="vendor_promo">
        <thead>
            <tr>
                <th scope="col"> {{ __('vendor_promo.promo_code') }} </th>
                <th scope="col">{{ __('vendor_promo.promo_desc') }}</th>
                <th scope="col">Action</th>
            </tr>
        </thead>
        <tbody>
            @foreach ($vendor_promo as $promo)
                <tr>
                    ...
                    <td>
                        <div class="flex space-x-3">
                            <div>
                                <a href="{{ route('vendor-promotion.edit', $promo->promo_code) }}">
                            ...
                            <div>
                                <form action="{{ route('vendor-promotion.destroy', $promo->promo_code) }}" method="post"
...
@section('js-scripts')
    <script>
        if (document.getElementById("vendor_promo") && typeof simpleDatatables.DataTable !== 'undefined') {
            const dataTable = new simpleDatatables.DataTable("#vendor_promo", {
                searchable: true,
                sortable: false
            });
        }
