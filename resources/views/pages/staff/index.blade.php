...
@section('add-button')
    <div>
        <a href="{{ route('staff.create') }}" class="add-button">
...
@section('table-section')
    <table class="table-data" id="staff_table">
        <thead>
            <tr>
                <th scope="col"> {{ __('staff.staff_id') }} </th>
                <th scope="col"> {{ __('staff.staff_name') }} </th>
                        ...
                        <div class="flex space-x-3">
                            ...
                            <div>
                                <form action="{{ route('staff.destroy', $staff->staff_id) }}" method="post"
                                    id="delete-form-{{ $staff->staff_id }}">
                                    @csrf
                                    @method('DELETE')
                                    <button id="del-button" class="del-button" data-item-id="{{ $staff->staff_id }}"
...
@section('js-scripts')
    <script>
        if (document.getElementById("staff_table") && typeof simpleDatatables.DataTable !== 'undefined') {
            const dataTable = new simpleDatatables.DataTable("#staff_table", {
                searchable: true,
                sortable: false
            });
        }
