...
@section('add-button')
    <div>
        <a href="{{ route('branch.create') }}" class="add-button">
...
@section('table-section')
    <table class="table-data" id="branch_table">
        <thead>
            <tr>
                <th scope="col">{{ __('branch.branch_id') }}</th>
                <th scope="col">{{ __('branch.branch_name') }}</th>
                        ...
                        <div class="flex space-x-3">
                            ...
                            <div>
                                <form action="{{ route('branch.destroy', $branch->branch_id) }}" method="post"
                                    id="delete-form-{{ $branch->branch_id }}">
                                    @csrf
                                    @method('DELETE')
                                    <button id="del-button" class="del-button" data-item-id="{{ $branch->branch_id }}"
...
@section('js-scripts')
    <script>
        if (document.getElementById("branch_table") && typeof simpleDatatables.DataTable !== 'undefined') {
            const dataTable = new simpleDatatables.DataTable("#branch_table", {
                searchable: true,
                sortable: false
            });
        }
